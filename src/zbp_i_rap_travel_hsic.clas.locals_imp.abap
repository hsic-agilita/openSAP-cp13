...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_hsic IN LOCAL MODE
      ENTITY Travel
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_hsic( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_hsic=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
...
*                      %action-Edit         = COND #( WHEN update_granted = abap_true THEN if_abap_behv=>auth-allowed ELSE if_abap_behv=>auth-unauthorized )
  ...
  " Set the new overall status
    MODIFY ENTITIES OF zi_rap_travel_hsic IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
...
" Read relevant travel instance data
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcm_rap_hsic(
                                          severity = if_abap_behv_message=>severity-error
                                          textid   = zcm_rap_hsic=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_hsic(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_hsic=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF (  travel-EndDate < travel-BeginDate ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_hsic(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_hsic=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_hsic(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_hsic=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
  ...
  ENDMETHOD.
   ...
   METHOD is_update_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATHSIC'
        ID 'ZOSTATHSIC' FIELD overall_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATHSIC'
        ID 'ZOSTATHSIC' DUMMY
